            ...
            <div class="card" style="margin: 15px 0;">
                <div class="card-content">
                    <div class="card-title d-flex">
                        ...
                        <div>
                            <a href="{{ route('financial.registration.payment-plans.create') }}" 
                        ...
                        <tbody>
                            @forelse($paymentPlans as $plan)
                                <tr>
                                    <td>{{ $plan->nome }}</td>
                                    <td>
                                        <span class="chip {{ $plan->tipo == 'venda' ? 'blue' : 'orange' }} white-text">
                                            {{ $plan->tipo == 'venda' ? 'Venda' : 'Compra' }}
                                        </span>
                                    </td>
                                    <td>{{ $plan->paymentMethod->nome }}</td>
                                    <td>{{ $plan->parcelas }}x</td>
                                    <td>{{ number_format($plan->taxa, 2) }}%</td>
                                    <td>
                                        <span class="chip {{ $plan->ativo ? 'green' : 'red' }} white-text">
                                    ...
                                    <td>
                                        <div class="buttons-actions">
                                            <a href="#modal-simulate" 
                                               ...
                                               onclick="setPlanForSimulation({{ $plan->id }})"
                                               title="Simular">
                                                <i class="material-icons">calculate</i>
                                            </a>
                                            <a href="{{ route('financial.registration.payment-plans.edit', $plan) }}" 
                                               class="btn-floating waves-effect waves-light amber"
                                               title="Editar">
                                                <i class="material-icons">edit</i>
                                            </a>
                                            <button type="button"
                                                    onclick="deleteItem('{{ route('financial.registration.payment-plans.destroy', $plan) }}')"
                    ...
                    <div class="row">
                        <div class="col s12">
                            {{ $paymentPlans->links('vendor.pagination.materialize') }}
                        </div>
                    </div>
                </div>
            </div>
...
<!-- Modal Simulação -->
@include('financial.registration.payment-plans._simulate_modal')

<!-- Modal Confirmação Exclusão -->
@include('shared._confirm_delete_modal')
