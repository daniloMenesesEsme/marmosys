...
@section('title', 'Nova Área de Atendimento')

@section('content')
<div class="container">
    <div class="row mb-3">
        <div class="col s12">
            <h4>Nova Área de Atendimento</h4>
        </div>
    </div>
                    ...
                    <form action="{{ route('service-areas.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="input-field col s12 m6">
                                <select name="location_id" id="location_id" required>
                                    <option value="" disabled {{ !old('location_id') && !request('location_id') ? 'selected' : '' }}>Selecione uma localidade</option>
                            ...
                            <div class="input-field col s12 m6">
                                <select name="establishment_type_id" id="establishment_type_id">
                                    ...
                                    @foreach($establishmentTypes as $type)
                                        <option value="{{ $type->id }}" {{ (old('establishment_type_id') == $type->id || request('establishment_type_id') == $type->id) ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            <div class="input-field col s12 m6">
                                ...
                                <span class="helper-text">Pode ser atribuído posteriormente</span>
                                @error('seller_id')
                                    <span class="red-text">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="input-field col s12 m3">
                                <input id="goal_amount" type="number" step="0.01" min="0" name="goal_amount" value="{{ old('goal_amount') }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkExistingArea() {
            ...
            if (locationId) {
                fetch(`/api/check-service-area?location_id=${locationId}&establishment_type_id=${typeId}`)
        ...
        // Verifica inicialmente se houver valores pré-selecionados
        if (locationSelect.value && (typeSelect.value || typeSelect.value === '')) {
            checkExistingArea();
        }
    });
</script>
